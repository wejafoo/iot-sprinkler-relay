...
void setup() {
  ...
  server.on("/led/on"                , []() { SetPin(led_pin, led_on  );       });
  server.on("/led/off"               , []() { SetPin(led_pin, led_off );       });
  server.on("/pod/1/zone/1/mode/on"  , []() { SetPin(relay_pin_1, relay_on  ); });
  server.on("/pod/1/zone/1/mode/off" , []() { SetPin(relay_pin_1, relay_off ); });
  server.on("/pod/1/zone/2/mode/on"  , []() { SetPin(relay_pin_2, relay_on  ); });
  server.on("/pod/1/zone/2/mode/off" , []() { SetPin(relay_pin_2, relay_off ); });
  server.on("/pod/1/zone/3/mode/on"  , []() { SetPin(relay_pin_3, relay_on  ); });
  server.on("/pod/1/zone/3/mode/off" , []() { SetPin(relay_pin_3, relay_off ); });
  server.on("/pod/1/zone/4/mode/on"  , []() { SetPin(relay_pin_4, relay_on  ); });
  server.on("/pod/1/zone/4/mode/off" , []() { SetPin(relay_pin_4, relay_off ); });
